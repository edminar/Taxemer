...
def train_model_by_img(name):
    ...
    if not os.path.exists("dataset"):
        print("[ОШИБКА] нет каталога 'dataset'")
        sys.exit()
    ...
    for(i, image) in enumerate(images):
        print(f"[+] обработка img {i + 1}/{len(images)}")
        print(image)
    ...
    while True:
        ...
        if ret:
            ...
            if frame_id % multiplier == 0:
                ...
                #print(f"Сделайте скриншотов {count}")
                count += 1

            if k == ord(" "):
                cv2.imwrite(f"dataset_from_video/{count}_extra_scr.jpg", frame)
                #print(f"Сделал дополнительный скриншот {count}")
                count += 1
            elif k == ord("q"):
                print("Q нажми, для закрытия")
                break
